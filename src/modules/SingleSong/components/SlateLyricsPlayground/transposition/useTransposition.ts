 ...
 *
 * Різниця — режим: капо діє лише в читанні, у редагуванні й примітках воно
 * знімається (див. таблицю в `mode.tsx`). Документ при цьому не чіпаємо:
 * значення капо лишається, просто не застосовується. Тому все, що ПОКАЗУЄ
 * або ГРАЄ акорди, ходить сюди, а не в `resolveTransposition` напряму.
 *

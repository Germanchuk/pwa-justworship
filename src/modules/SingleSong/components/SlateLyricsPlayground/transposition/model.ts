 ...
 *   v1:          показ транспонованих акордів ЛИШЕ у read/view mode + плейбек
 *
 ...
 * `resolveTransposition` повертає myCapo=0, тож показ/блокування/плейбек
 * автоматично поводяться так, ніби капо нема (акорди спільні, редагування дозволене).
 */
...
/**
 * ─────────────────────────────────────────────────────────────────────────
 * TODO (відкладена ітерація, ПОЗА обсягом v1): "редагування у своїй тональності".
 *
 * Коли капо-юзер у режимі редагування пише акорди, він хоче писати у СВОЇЙ
 * тональності (effectiveKey, напр. A), а в документ вони мають приходити у
 * СПІЛЬНІЙ (songKey, напр. C). Тобто на ввід — транспозиція ВГОРУ на capo
 * (зворотна до показу), на показ — вниз на capo.
 *
 * У v1 цього немає: per-user показ працює лише у read/view mode; у режимі
 * редагування капо-юзер бачить/пише спільні акорди.
 * ─────────────────────────────────────────────────────────────────────────
 */
